...
switch ($type) {
		...
		switch (last_action()) {
			...
			default:
				$nav->navs[] = array(
					...
					"<a href='?mod=$type&action=details&id=$id'>{$lang['details']}</a> ($title)",
					$lang['postcomment']
				);
			break;
			...
			case "admin";
				$nav->navs[] = array(
					...
					"<a href='?mod=comments&action=admin&id=$id&type=$type'>{$lang['comments']}</a> ($title)",
					$lang['create']
				);
			break;
		}
		...
		switch (last_action()) {
			...
			case "admin";
				$nav->navs[] = array(
					"<a href='?mod=$type'>{$lang[$type]}</a>",
					"<a href='?mod=$type&action=admin'>{$lang['admin']}</a>",
					"<a href='?mod=comments&action=admin&id=$id&type=$type'>{$lang['comments']}</a>".
					" ({$clan_inf['tag']} - {$match['match_type_name']})",
					$lang['create']
				);
			break;
	...
	case "reports":
		...
		switch (last_action()) {
			...
			default:
				$nav->navs[] = array(
					"<a href='?mod=$type&action=details&type=$origin&id=$id&type_id=$origin_id'>".
			...
			case "admin";
				$nav->navs[] = array(
					"<a href='?mod=comments&action=admin&id=$id&type=$type".
	...
	case "polls":
		...
		$title = $core_db->lookup(
			"SELECT title FROM polls_data WHERE id = '$id'",
			"Lookup poll title."
		);
